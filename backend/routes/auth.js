...
passport.use('google', new GoogleStrategy({
  ...
  try {
    ...
    let tipo = 'alumno'; // Valor por defecto
    ...
    // Puedes añadir más condiciones según necesites

    const { rows } = await pool.query(`
      INSERT INTO usuarios (nombre, correo, tipo, google_id, avatar_url)
      VALUES ($1, $2, $3, $4, $5)
      ON CONFLICT (correo) 
      DO UPDATE SET google_id = $4, avatar_url = $5
      RETURNING *
    `, [displayName, email, tipo, id, photos[0]?.value]); // Usa la variable tipo
